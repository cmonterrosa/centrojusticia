...
class User < ActiveRecord::Base
  ...
  ##### Puntuaciones #######
     ...
     def num_orientaciones_periodo(inicio=Time.now,fin=Time.now)
      ...
      AND e.clave in ('comp-conc', 'no-compar', 'mate-asig', 'tram-admi', 'fech-asig', 'camb-sesi', 'tram-canc', 'tram-conc', 'proc-conv', 'en-sesion', 'conv-vali', 'adj-car', 'adj-eli', 'meca-asig')
  ...
  def estatus_actual
    ...
    def expedientes_sin_concluir(anio=Time.now.year)
     return Tramite.find_by_sql("SELECT * from tramites where id not in (SELECT tramite_id from concluidos) AND
      ...
      (mediador_id=#{self.id}) AND (cancel_user IS NULL OR cancel=0)) AND (anio >=#{anio} OR anio = 2016)")
    end
