...
class ComparecenciasController < ApplicationController
  ...
  def generar_pdf_observaciones
    ...
    if (@involucrado && @involucrado.perfil == "INVOLUCRADO" && @comparecencia)
       ...
       param["P_HORARIO_DOMICILIO_PARTICULAR"]= (@involucrado.horario_disponible_domicilio_particular)? {:tipo=>"String", :valor=>@involucrado.horario_disponible_domicilio_particular} : {:tipo=>"String", :valor=> "NO EXISTE INFORMACIÓN"}
       param["P_HORARIO_DOMICILIO_LABORAL"]=  (@involucrado.horario_disponible_domicilio_laboral)? {:tipo=>"String", :valor=>@involucrado.horario_disponible_domicilio_laboral} : {:tipo=>"String", :valor=>"NO EXISTE INFORMACIÓN"}
  ...
  def generar_pdf_acta_comparecencia
      ...
      if @solicitante && @solicitante.perfil == "SOLICITANTE"
        ...
        param["P_REFERENCIA_DOMICILIARIA_LABORAL"]=(@solicitante.referencias_domiciliares_laboral)? {:tipo=>"String", :valor=>clean_string(@solicitante.referencias_domiciliares_laboral)} : ""
        ...
        param["P_HORARIO_DOMICILIO_LABORAL"]=  (@solicitante.horario_disponible_domicilio_laboral)? {:tipo=>"String", :valor=>@solicitante.horario_disponible_domicilio_laboral} : ""
    ...
    if @sesion.tramite &&  @sesion.save
     ...
     def change_to_orientacion
       ...
       if @user_success && @time_success
          ...
          flash[:error] = "No se pudo cambiar a solo orientacion, no le pertenece o tiene más de 180 días que capturó comparecencia, contacte al administrador"
