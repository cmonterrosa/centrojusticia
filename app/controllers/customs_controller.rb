class CustomsController < ApplicationController
  ...
  def show_calendario
     @sesion = Sesion.new
     @sesiones = (params[:id] == "all") ? Sesion.find(:all, :select=> ["s.*"], :joins => "s, horarios h", :conditions => ["cancel is NULL AND s.horario_id=h.id"], :order => "s.fecha, h.hora,h.minutos") :  Sesion.find(:all, :conditions => ["fecha is not NULL and (mediador_id = ? OR comediador_id = ?)", current_user.id, current_user.id], :order => "fecha")
